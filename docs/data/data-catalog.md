...
1. **データは通常の作業対象・追跡候補である。** GitHub未収録だけを理由に作業を省略せず、100MB未満のデータ・派生物・成果物を一律にgit管理外にしない。100MB以上、秘密情報、又は再配布不可の原本だけを個別に扱う。
...
| `FirmList\` | 66 | — | 企業リスト |
| `CorporateGovernance\` | 22 | `CorporateGovernance.parquet` / `.csv` | **CG 報告書パネル**（企業×提出暦年、2015–2025、4,490 社、72 列） |

### コーポレート・ガバナンス（`eol_cg_panel`）

eol の CG 検索（`CgSearch.php`）から取得。検索は 3,000 件で打ち切られるため、
**提出日＝暦年 × 「最新データのみを対象にする」ON × 決算期（3月／それ以外）**で
22 本に分割して DL する（`cg_{年}_{mar|other}.csv`）。提出日の年は **2015 年以降**しか
選べない（eol の制約）。生成は `data-pipeline/eol/build_cg_panel.py`。

- 粒度は **CG 報告書の提出暦年**（決算期ではない）。組織形態の変更は定時株主総会で
  決議されるので、暦年 Y の報告書が監査等委員会設置会社を初めて示したなら総会は Y 年。
  3月決算なら新体制下の最初の完全な事業年度は Y+1 年 3 月期。
- 主な列: 組織形態（監査役設置／監査等委員会／指名委員会等）、取締役・社外取締役・
  独立役員の人数、監査役会の有無・監査役人数・社外監査役、各委員会の委員数、
  親会社・支配株主、外国人株式保有比率、IR・買収防衛策 等。
- 派生列: `Org` / `IsKanaTou` / `IsShimei` / `IsKansayaku` / `OutsideShare` /
  `NMonitors`（監査役設置なら監査役人数、監査等委員会なら監査等委員数）/
  `KanaTouFirstCGYear`（監査等委員会への移行年。1,965 社が移行）。
- **NEEDS ディスク**にも CG 報告書関連（2015/2018/2019 各版）があるが、そちらは
  買い切りのスナップショット。連続パネルが要るときは eol 側を使う。
...
1. `data/`・`outputs/`は一律に除外しない。100MB未満の入力、派生データ、fixture、表、図、
   モデル、監査成果物は原則として追跡候補にする。
2. repoへ置いたマスタ類は`docs/data-sources.md`に「repo内パス｜原本キー｜内容」を1行記録する。
3. GB級原本は必要列・必要行を`data/processed/*.parquet`等へ抽出し、原本キー、抽出範囲、処理コードを
   記録する。抽出物が100MB未満なら追跡候補にし、100MB以上又は再配布不可ならその実在ファイルだけを個別に扱う。
